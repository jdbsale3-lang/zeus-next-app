...
import { Copy as IconCopyOutlined } from "lucide-react";
import { ExternalLink as IconExternalLinkOutlined } from "lucide-react";
 ...
 * `trigger` and the real `generation` data. Another agent can wire this
 ...
 * Live mode shows only metadata and actions actually provided by the caller;
 * the visual-preview defaults exist only behind explicit `demo` mode.
 */

export interface GenerationDetail {
  /** Durable media source (image or video) from the live flow or app-owned assets. */
  src: string;
 ...
 * `detailRows` to add / remove / reorder rows — omit it to derive rows only
 * from metadata present on the generation.
 */
...
export interface GenerationDetailAction {
  ...
  label: string;
  icon: IconGlyph;
  /** Required in live mode so a visible control can never be a no-op. */
  onSelect: () => void;
  /** Render as a square icon-only button (no label). Default `false`. */
  iconOnly?: boolean;
  /** Optional toggle state for actions such as Like. */
  pressed?: boolean;
}
...
export interface GenerationDetailPrimaryAction {
  label: ReactNode;
  icon?: IconGlyph;
  /** Required in live mode so the primary CTA always performs its label. */
  onSelect: () => void;
}

interface GenerationDetailModalCommonProps {
   ...
   * Last used). Same fixed layout — only the rows change. Omit to derive rows
   * from fields present on the live generation.
   */
  detailRows?: GenerationDetailRow[];
  /**
   * The full-width lime CTA. It renders only when supplied with a working
   * `onSelect`; omit it when the workflow is unavailable.
   */
  primaryAction?: GenerationDetailPrimaryAction | null;
  /**
   * The footer action buttons below the CTA. Omit for the functional default
...
/** Live viewers require real data; the visual scaffold fallback is explicit. */
export type GenerationDetailModalProps = GenerationDetailModalCommonProps &
  ({ demo: true; generation?: never } | { demo?: false; generation: GenerationDetail });
...
// to find every site.
const DEMO_GENERATION: GenerationDetail = {
  src: "/presets/how-product-works.png",
  mediaType: "image",
  aspectRatio: 2 / 3,
  author: { name: "Current user", role: "Creator" },
  status: "Uploaded",
  fileType: "JPG",
  size: "2.4 MB",
...
async function shareGeneration(src: string, prompt: string): Promise<void> {
  const url = new URL(src, window.location.href).href;
  if (navigator.share != null) {
    await navigator.share({ title: "Generated media", text: prompt, url });
    return;
  }
  await copyText(`${prompt}\n${url}`);
}

function MoreActionMenu({ src, prompt }: { src: string; prompt: string }) {
  return (
    <Dropdown.Root>
      <Dropdown.Trigger
        render={
          <Button
            ...
            start={<Icon as={IconDotGrid1x3HorizontalOutlined} size="sm" />}
          />
        }
      />
      <Dropdown.Content surface="solid" side="top" align="end" sideOffset={6}>
        <Dropdown.Item
          value="copy-prompt"
          start={<Icon as={IconCopyOutlined} size="sm" />}
          title="Copy prompt"
          onSelect={() => void copyText(prompt)}
        />
        <Dropdown.Item
          value="open-original"
          start={<Icon as={IconExternalLinkOutlined} size="sm" />}
          title="Open original"
          onSelect={() => window.open(new URL(src, window.location.href).href, "_blank", "noopener,noreferrer")}
        />
      </Dropdown.Content>
    </Dropdown.Root>
  );
...
function InfoPanel({
  ...
  // Derive metadata from real generation fields and keep default actions functional.
  const defaultRows: GenerationDetailRow[] = [];
  if (data.status != null) {
    defaultRows.push({
      id: "status",
      label: "Status",
  ...
  if (data.fileType != null) defaultRows.push({ id: "type", label: "Type", value: data.fileType });
  if (data.size != null) defaultRows.push({ id: "size", label: "Size", value: data.size });
  if (data.uploadedAt != null) {
    defaultRows.push({ id: "uploaded", label: "Uploaded", value: data.uploadedAt });
  }
  if (data.lastUsedAt != null) {
    defaultRows.push({ id: "lastUsed", label: "Last used", value: data.lastUsedAt });
  }
  const rows = detailRows ?? defaultRows;
  const primary = primaryAction?.onSelect != null ? primaryAction : null;
  const defaultActions: GenerationDetailAction[] = [
    {
      id: "download",
      label: "Download",
    ...
    },
    {
      ...
      iconOnly: true,
      pressed: liked,
      onSelect: () => setLiked((current) => !current),
    },
    {
      id: "share",
      label: "Share",
      icon: IconArrowShareRightOutlined,
      iconOnly: true,
      onSelect: () => {
        void shareGeneration(data.src, data.prompt ?? "").catch((error: unknown) => {
  ...
  const footerActions = (actions ?? defaultActions)
    .map((action) =>
      action.id === "download" && action.onSelect == null
        ? { ...action, onSelect: () => void downloadMedia(data.src) }
        : action,
    )
    .filter((action) => action.onSelect != null);
  const showDefaultMore = actions === undefined;
  ...
  return (
    <aside className="flex h-full w-full flex-col gap-2 p-2">
      ...
      {primary != null || footerActions.length > 0 || showDefaultMore ? (
        <div className="flex flex-col gap-2 p-2">
          ...
          {footerActions.length > 0 ? (
            <div className="flex items-center gap-2">
              ...
              {showDefaultMore ? <MoreActionMenu src={data.src} prompt={data.prompt ?? ""} /> : null}
            </div>
          ) : null}
        </div>
      ) : null}
...
export function GenerationDetailModal(props: GenerationDetailModalProps) {
  const { trigger, detailRows, primaryAction, actions, open, onOpenChange, defaultOpen } = props;
  const generation = props.demo ? DEMO_GENERATION : props.generation;
  const data = generation;
  const backdropSource = data.mediaType === "video" ? data.poster : data.src;
