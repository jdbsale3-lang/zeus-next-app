 ...
 */
interface UserGenerationsCommonProps {
  /** Heading displayed beside the gallery density control. */
  title?: ReactNode;
  /** Shared screen empty state using representative generations from this app. */
  emptyState?: ScreenEmptyStateContent;
}

/** Live feeds require real items; demo history is available only by explicit opt-in. */
export type UserGenerationsProps = UserGenerationsCommonProps &
  (
...
export function UserGenerations(props: UserGenerationsProps) {
  return props.demo ? (
    <JustifiedGallery
      ...
      grouped={false}
      title={props.title}
      {...(props.emptyState != null ? { emptyState: props.emptyState } : {})}
    />
  ) : (
    <JustifiedGallery
      items={props.items}
