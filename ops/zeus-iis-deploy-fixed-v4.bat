@echo off
rem ============================================================
rem  ZEUS AI - IIS deploy script FIXED v4 (rollback-pair validated) (ADMIN prompt)
...
echo [%date% %time%] DEPLOY START (fixed v3) >> "%LOG%"
...
echo.
rem optional SHA-256 confirmation (Windows 10+)
certutil -hashfile "%SRC%\zeus.html" SHA256 | findstr /I "%SEALED%" >nul && echo SHA256 MATCH - sealed fingerprint confirmed
endlocal
exit /b 0
