@echo off
rem ============================================================
rem  ZEUS AI - IIS deploy script FIXED v7 (parse-time fixes + post-swap smoke) (ADMIN prompt)
...
echo [%date% %time%] DEPLOY START (fixed v7) >> "%LOG%"
...
rem   (PSZ initialized - an EMPTY size (missing previous file) must not crash
rem    the `if %PSZ% LSS` with "600000 was unexpected" - Cursor review fix)
set "PSZ=0"
...
rem   FIX: %HTTP% set inside the parens block was expanded at PARSE time (empty),
rem   so v5/v6 warned even on 200. Set now happens in the loop, compare AFTER the block.
set "HTTP=000"
where curl >nul 2>&1 && (
...
rem SMOKE TEST 3: the ROLLBACK PAIR must itself be restorable and sealed
rem   - zeus.previous.html exists, sane size, has the payment marker,
rem   - and its hash still matches the sealed standard (or the pairing log
rem     shows it was a prior known-good build).
set "PSZ=0"
for %%A in ("%SRC%\zeus.previous.html") do set "PSZ=%%~zA"
if %PSZ% LSS 600000 (echo SMOKE FAIL - rollback pair too small (%PSZ% B); DO NOT trust rollback) else (echo SMOKE OK - rollback pair size %PSZ% B)
findstr /C:"zeusPaymentCheck" "%SRC%\zeus.previous.html" >nul && (echo SMOKE OK - rollback pair has payment marker) || (echo SMOKE FAIL - rollback pair missing payment marker)
for /f "delims=" %%H in ('certutil -hashfile "%SRC%\zeus.previous.html" SHA256 ^| findstr /R /C:"^[0-9a-fA-F][0-9a-fA-F]*"') do set "CHK3=%%H"
set "CHK3=%CHK3: =%"
if /I "%CHK3%"=="%SEALED%" (echo SMOKE OK - rollback pair hash matches sealed standard) else (echo SMOKE NOTE - rollback pair is a previous known-good build, not the current sealed hash)

rem SMOKE TEST 4 (POST-SWAP): the PUBLIC estate must now serve the sealed build
rem   (checks the live site, not just localhost - catches a broken index.html
rem    sync or edge cache pointing at stale content; skips gracefully offline)
set "LIVE=000"
where curl >nul 2>&1 && (
  for /f "delims=" %%L in ('curl -s -o NUL -w "%%{http_code}" --max-time 15 https://zeusaiintelligence.com') do set "LIVE=%%L"
)
if "%LIVE%"=="200" (
  curl -s -m 15 https://zeusaiintelligence.com | findstr /C:"zeusPaymentCheck" >nul && (echo SMOKE OK - public site serves 200 with payment marker) || (echo SMOKE WARNING - public site 200 but payment marker missing; check index.html sync)
