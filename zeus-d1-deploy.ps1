# ============================================================
#  ZEUS D1 GOVERNANCE DEPLOYER v1 - executable runbook
...
#  ---- rename to zeus-d1-deploy.ps1 (remove .txt) ----
# ============================================================
...
$out = & npx wrangler d1 execute zeus-db --command $q --remote 2>&1
$out | Write-Host
$found = @()
...
if ($found.Count -eq 3) {
  ...
  Run "apply 0010" "npx wrangler d1 execute zeus-db --file=app/migrations/0010_governance.sql --remote"
} else {
  OK "Case A: $($found.Count)/3 present - apply 0011 then 0010"
  Run "apply 0011" "npx wrangler d1 execute zeus-db --file=app/migrations/0011_aegis_fix.sql --remote"
  Run "apply 0010" "npx wrangler d1 execute zeus-db --file=app/migrations/0010_governance.sql --remote"
}

# 3) verify
Run "verify policies" "npx wrangler d1 execute zeus-db --command `"SELECT id, rule FROM policy_rules LIMIT 5`" --remote"
Run "verify guards" "npx wrangler d1 execute zeus-db --command `"SELECT name FROM sqlite_master WHERE type='trigger' AND name LIKE 'guard_audit_%'`" --remote"
